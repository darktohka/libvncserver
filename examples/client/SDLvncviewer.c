...
struct {
  int sdl;
  int rfb;
} buttonMapping[] = {{1, rfbButton1Mask}, {2, rfbButton2Mask},
                     {3, rfbButton3Mask}, {4, rfbButton4Mask},
                     {5, rfbButton5Mask}, {0, 0}};

struct {
  char mask;
  int bits_stored;
} utf8Mapping[] = {{0b00111111, 6}, {0b01111111, 7}, {0b00011111, 5},
...
static rfbBool resize(rfbClient *client) {
  ...
  if (!sdlWindow) {
    sdlWindow = SDL_CreateWindow(client->desktopName, width, height, sdlFlags);
    if (!sdlWindow)
  ...
  SDL_SetRenderLogicalPresentation(
      sdlRenderer, width, height, SDL_LOGICAL_PRESENTATION_LETTERBOX,
      SDL_SCALEMODE_LINEAR); /* this is a departure from the SDL1.2-based
                                version, but more in the sense of a VNC viewer
                                in keeeping aspect ratio */

  /* (re)create the texture that sits in between the surface->pixels and the
   * renderer */
  if (sdlTexture)
...
static rfbKeySym SDL_key2rfbKeySym(SDL_KeyboardEvent *e) {
  ...
  /* SDL_TEXTINPUT does not generate characters if ctrl is down, so handle those
   * here */
...
/* UTF-8 decoding is from https://rosettacode.org/wiki/UTF-8_encode_and_decode
 * which is under GFDL 1.2 */
...
static void update(rfbClient *cl, int x, int y, int w, int h) {
  ...
  SDL_Rect r = {x, y, w, h};
  if (SDL_UpdateTexture(sdlTexture, &r, sdl->pixels + y * sdl->pitch + x * 4,
                        sdl->pitch) < 0)
